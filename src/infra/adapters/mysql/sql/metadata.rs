...
mod tests {
    ...
    fn metadata_queries_escape_literals_and_preserve_scope_conditions() {
        ...
        assert_eq!(
            table_detail_metadata_sql.matches("JSON_ARRAYAGG").count(),
            5
        );
        assert_eq!(table_detail_metadata_sql.matches("JSON_OBJECT").count(), 6);
        assert_eq!(
        ...
        assert!(
            table_detail_metadata_sql
                .contains("tc.CONSTRAINT_SCHEMA = DATABASE() AND tc.TABLE_SCHEMA = c.TABLE_SCHEMA")
        );
        assert!(table_detail_metadata_sql.contains(
            "rc.CONSTRAINT_SCHEMA = tc.CONSTRAINT_SCHEMA AND rc.TABLE_NAME = tc.TABLE_NAME"
        ));
        assert!(table_detail_metadata_sql.contains(&format!(
            "AND EVENT_OBJECT_SCHEMA = {quoted_schema} AND EVENT_OBJECT_TABLE = {quoted_table}"
        )));
        assert!(table_detail_metadata_sql.ends_with(")) AS METADATA_JSON"));
