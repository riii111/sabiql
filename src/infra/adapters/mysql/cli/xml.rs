...
use super::process::{has_mysql_cli_error, trace_mysql_error};

pub(crate) const MYSQL_RESULTSET_START: &[u8] = b"<resultset";
pub(crate) const MYSQL_RESULTSET_END: &[u8] = b"</resultset>";

#[derive(Debug, Default)]
pub(crate) struct MysqlResultsetFrameScanner {
...
impl MysqlResultsetFrameScanner {
    pub(crate) fn frame_bounds(&mut self, buffer: &[u8]) -> Option<(usize, usize)> {
    ...
    pub(crate) fn take(&mut self, buffer: &mut Vec<u8>) -> Option<Vec<u8>> {
        let bounds = self.frame_bounds(buffer)?;
        Some(self.take_bounds(buffer, bounds))
    }

    pub(crate) fn take_bounds(
        &mut self,
        buffer: &mut Vec<u8>,
...
#[cfg(any(unix, test))]
pub(crate) fn take_mysql_pty_resultset_frame(
    buffer: &mut Vec<u8>,
...
#[cfg(any(not(unix), test))]
pub(crate) fn take_mysql_resultset_frame_after_error_check(
    buffer: &mut Vec<u8>,
    error_output: &[u8],
    scanner: &mut MysqlResultsetFrameScanner,
) -> Result<Option<Vec<u8>>, DbOperationError> {
...
pub(crate) fn find_bytes_from(haystack: &[u8], needle: &[u8], start: usize) -> Option<usize> {
    haystack
        .get(start..)?
        .windows(needle.len())
...
pub(crate) fn decode_mysql_xml_reference(
...
pub(crate) fn parse_mysql_xml(xml: &[u8]) -> Result<MysqlResultSet, DbOperationError> {
...
impl MysqlField {
    ...
    pub(crate) fn finish_raw(self) -> (String, String) {
        let value = if self.is_null {
            String::new()
        } else {
            self.value
...
pub(crate) fn parse_mysql_field(
