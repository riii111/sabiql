...
fn sql_modal_keyword_and_number_use_syntax_colors() {
    ...
    state
        .sql_modal
        .set_editor_content_for_test("SELECT 42".to_string());
...
fn sql_modal_string_comment_and_operator_use_syntax_colors() {
    ...
    state
        .sql_modal
        .set_editor_content_for_test("SELECT 'x'::text -- note".to_string());
...
fn test_contrast_theme_applies_sql_syntax_colors() {
    ...
    state
        .sql_modal
        .set_editor_content_for_test("SELECT 'x' + 42 -- note".to_string());
    state.sql_modal.set_status_for_test(SqlModalStatus::Editing);
...
fn sql_modal_insert_cursor_uses_display_width_for_wide_chars() {
    ...
    state
        .sql_modal
        .set_editor_content_with_cursor_for_test(content, 2);
...
fn sql_modal_insert_cursor_advances_visual_row_when_line_wraps() {
    ...
    state
        .sql_modal
        .set_editor_content_with_cursor_for_test(content, 18);
...
fn sql_modal_unterminated_string_keeps_string_highlight() {
    ...
    state
        .sql_modal
        .set_editor_content_for_test("SELECT 'unterminated".to_string());
    state.sql_modal.set_status_for_test(SqlModalStatus::Editing);
...
fn sql_modal_unterminated_block_comment_keeps_comment_highlight() {
    ...
    state
        .sql_modal
        .set_editor_content_for_test("SELECT /* pending".to_string());
    state.sql_modal.set_status_for_test(SqlModalStatus::Editing);
...
fn sql_completion_popup_uses_injected_theme_styles() {
    ...
    state
        .sql_modal
        .set_editor_content_for_test("SELECT ".to_string());
    state.sql_modal.set_status_for_test(SqlModalStatus::Editing);
