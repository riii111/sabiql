...
use crate::domain::{
    ConnectionId, DatabaseMetadata, MetadataState, QueryResult, QuerySource, Table, TableKindInfo,
};
...
mod connection_state_tests {
    ...
    fn mysql_switch_invalidates_previous_metadata_failure() {
        ...
        let mysql_effects = reduce(&mut state, &Action::SwitchConnection(mysql.clone())).unwrap();
    ...
    fn stale_mysql_probe_completion_is_ignored_after_switch() {
        ...
        let first_effects = reduce(&mut state, &Action::SwitchConnection(first.clone())).unwrap();
