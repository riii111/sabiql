...
mod tests {
    ...
    mod cache_tests {
        ...
        #[test]
        fn keeps_mysql_table_detail_retry_alive_when_closing_failed_switch_error() {
            let mut state = AppState::new("test".to_string());
            let first = ConnectionTarget {
                id: ConnectionId::from_string("mysql-a"),
                dsn: "mysql://user@localhost:3306/a".to_string(),
            ...
            let initial_detail_run_id = state.session.begin_table_detail_run();
            ...
            .expect("probe failure should retry the active table detail");
            let retry_run_id = retry_effects
                ...
                .find_map(|effect| match effect {
                    Effect::FetchTableDetail {
                        ...
                        run_id,
                        ..
                    } if dsn == &first.dsn && *effect_generation == generation => Some(*run_id),
                    _ => None,
                })
                .expect("probe failure should reissue the table-detail request");
            assert_ne!(retry_run_id, initial_detail_run_id);
