...
pub(crate) enum EditGuardrailError {
...
pub(crate) struct BulkDeletePreviewResult {
    pub preview: WritePreview,
    pub target_page: usize,
    pub target_row: Option<usize>,
}

pub(crate) fn reject_sqlite_null_pk(
...
pub(crate) fn editable_preview_base(
    state: &AppState,
...
pub(crate) fn ensure_column_writable(
    state: &AppState,
    column_name: &str,
...
pub(crate) fn build_bulk_delete_preview(
    state: &AppState,
...
pub(crate) fn deletion_refresh_target_bulk(
    row_count: usize,
    deleted_count: usize,
...
pub(crate) fn find_text_matches(content: &str, query: &str) -> Vec<usize> {
    if query.is_empty() {
        return Vec::new();
    }
...
pub(crate) fn validate_field(state: &mut ConnectionSetupState, field: ConnectionField) {
...
pub(crate) fn validate_all(state: &mut ConnectionSetupState) {
