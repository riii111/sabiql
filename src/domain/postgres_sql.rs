...
mod tests {
    ...
    #[test]
    fn builds_update_sql_with_postgres_literals() {
        assert_eq!(
            build_update_sql(
                "public",
                "users",
    ...
    fn builds_bulk_delete_sql_with_row_grouping() {
        assert_eq!(
            build_bulk_delete_sql(
                ...
                "users",
                &[
                    vec![("id".into(), QueryValue::text("1"))],
                    vec![("id".into(), QueryValue::text("2"))],
                ],
            ),
        ...
        assert_eq!(
            build_bulk_delete_sql(
                ...
                &[vec![
                    ("id".into(), QueryValue::Null),
                    ("tenant_id".into(), QueryValue::text("a")),
                ]],
            ),
            "DELETE FROM \"s\".\"t\"\nWHERE \"id\" IS NULL AND \"tenant_id\" = 'a';"
        );
    }
}

#[cfg(test)]
#[path = "postgres_sql_legacy_tests.rs"]
mod legacy_tests;
