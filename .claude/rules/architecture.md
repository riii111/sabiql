...
| vim-like ナビゲーションキー追加 | `app/nav_intent.rs` に NavIntent variant + map + resolve arm 追加。`keybindings/normal.rs` に display-only エントリ |
...
| Connection 系 reducer ロジック追加 | `app/reducers/connection/<feature>.rs` に追加。`connection/mod.rs` は dispatcher のみ |
