...
## CTAS / SELECT INTO 補正 (Stage 1.5)

- CTAS (`CREATE TABLE ... AS SELECT`) と SELECT INTO に対して psql は `SELECT n` を返す
- `correct_ctas_tags` が rollback filtering (Stage 2) の **前** に Select→Create 変換を行う
- これにより rolled-back な Create は Stage 2 で自然に破棄され、復活しない
