...
- すべての単一行テキスト入力フィールドは `TextInputState`（`app/text_input.rs`）で状態管理すること
...
キーバインドの整合性ルール（SSOT、派生表示、アンチパターン）は `interaction-contract.md` を参照。
キーバインド追加時は、そこに定義されたチェックリストに従うこと。
