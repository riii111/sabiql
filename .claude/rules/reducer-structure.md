...
- `SqlModalContext`: `content`, `cursor`, `completion.*`, `completion_debounce`, `prefetch_queue`, `prefetching_tables`, `failed_prefetch_tables`, `prefetch_started`（読み取り・queue の push/pop は許容。lifecycle flag の reset は aggregate API を使う）
