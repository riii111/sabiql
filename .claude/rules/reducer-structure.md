...
各 sub-dispatcher の `mod.rs` は dispatch chain を主責務とする。ロジックは各 `<feature>.rs` に配置する。サブモジュール共有の小さな `pub(super)` helper（計算・検証関数）やテストフィクスチャは `mod.rs` に置いてよいが、特定 feature のみが使う関数はその feature ファイルに置くこと。
...
Result 系 Action は `result/<feature>.rs` に追加する。`navigation/` には置かない。
