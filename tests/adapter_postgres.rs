...
mod metadata_fetch {
    ...
    async fn fetch_metadata_returns_schemas_and_tables() {
    ...
    #[tokio::test]
    #[ignore] // tracked: #133 — requires PostgreSQL
    async fn fetch_table_detail_returns_columns_and_indexes() {
        let adapter = PostgresAdapter::new();
        let dsn = test_dsn();

        let metadata = adapter.fetch_metadata(&dsn).await.unwrap();

        if let Some(first_table) = metadata.tables.first() {
            let detail = adapter
...
mod query_execution {
    ...
    #[tokio::test]
    #[ignore] // tracked: #133 — requires PostgreSQL
    async fn execute_preview_returns_rows_with_correct_columns() {
        let adapter = PostgresAdapter::new();
        let dsn = test_dsn();

        let metadata = adapter.fetch_metadata(&dsn).await.unwrap();

        if let Some(table) = metadata.tables.first() {
            let result = adapter
